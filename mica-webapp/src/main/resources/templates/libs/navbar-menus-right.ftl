<#macro rightmenus>
    ...
    <#elseif config??>
      ...
      <#if !config.openAccess || config.dataAccessEnabled>
        <li class="nav-item">
          <a class="nav-link" href="${pathPrefix!".."}/signin<#if rc.requestUri != "/" && !rc.requestUri?starts_with("/forgot-password") && !rc.requestUri?starts_with("/just-registered") && !rc.requestUri?starts_with("/error") && !rc.requestUri?starts_with("/signin")>?redirect=${rc.requestUri}</#if>"><@message "sign-in"/></a>
        </li>
      </#if>
