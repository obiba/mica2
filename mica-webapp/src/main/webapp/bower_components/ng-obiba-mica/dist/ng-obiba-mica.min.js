...
this.createCriteria=function(b,c){var d=a.defer(),f=(new CriteriaItemBuilder).type(RQL_NODE.AND).rqlQuery(b).build(),g={};if(!e.hasTargetQuery(b))return d.resolve({root:f,map:g}),d.promise;var h=[],i=this,j=0;return b.args.forEach(function(a){QUERY_TARGETS[a.name.toUpperCase()]&&h.push(a)}),h.forEach(function(a){i.builders(a.name,a,f,c).then(function(a){f.children.push(a.root),g=angular.extend(g,a.map),j++,j===h.length&&d.resolve({root:f,map:g})})}),d.promise},/**
       ...
       */
this.prepareCriteriaTermsQuery=function(a,b,c){function d(a,b,c){if(!a||!a.args)return null;if((a.name===RQL_NODE.IN||a.name===RQL_NODE.CONTAINS)&&a.args[0]===b)return a;for(var e=a.args.length;e--;){var f=d(a.args[e],b,c);f&&(a.args[e]=c)}}var f=(new RqlParser).parse(a),g=f.args.filter(function(a){return a.name===b.target}).pop();if(g){var h=new RqlQuery(RQL_NODE.EXISTS),i=e.criteriaId(b.taxonomy,b.vocabulary);h.args.push(i),d(g,i,h),g.args.push(e.aggregate([i])),g.args.push(e.limit(0,0))}return f.args.push(new RqlQuery(RQL_NODE.FACET)),c&&e.addLocaleQuery(f,c),f.serializeArgs(f.args)},this.prepareSearchQuery=function(a,b,c,d,g){var h=angular.copy(b),i=typeToTarget(a);e.addLocaleQuery(h,d);var j=f(i,h);j||(j=new RqlQuery(i),h.args.push(j));var k=c[i]||{from:0,size:10};return e.addLimit(j,e.limit(k.from,k.size)),g&&e.addSort(j,g),(new RqlQuery).serializeArgs(h.args)},/**
