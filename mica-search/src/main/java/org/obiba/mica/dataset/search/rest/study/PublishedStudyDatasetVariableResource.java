...
public class PublishedStudyDatasetVariableResource extends AbstractPublishedDatasetResource<StudyDataset>
  ...
  public Mica.DatasetVariableContingencyDto getContingency(@QueryParam("by") String crossVariable) {
    ...
    DatasetVariable crossVar = getDatasetVariable(datasetId, variableName, DatasetVariable.Type.Study, null);

    try {
      return dtos.asContingencyDto(studyTable, datasetService.getContingencyTable(dataset, var, crossVar)).build();
    } catch(Exception e) {
