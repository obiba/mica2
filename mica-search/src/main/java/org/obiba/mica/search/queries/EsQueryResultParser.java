  ...
  public List<AggregationResultDto> parseAggregations(@Nullable Aggregations defaults, @NotNull Aggregations queried) {
    ...
    IntStream.range(0, defaultAggs.size()).forEach(i -> {
      ...
      switch(aggType) {
        ...
        case "range":
          List<Range.Bucket> defaultRangeBuckets = ((Range) defaultAgg).getBuckets().stream().collect(Collectors.toList());
          List<Range.Bucket> queriedRangeBuckets = ((Range) queriedAgg).getBuckets().stream().collect(Collectors.toList());
  ...
  public List<AggregationResultDto> parseAggregations(@NotNull Aggregations aggregations) {
    ...
    aggregations.forEach(aggregation -> {
      ...
      switch(aggType) {
        ...
        case "range":
          ((Range) aggregation).getBuckets().stream().collect(Collectors.toList()).forEach(bucket -> {
