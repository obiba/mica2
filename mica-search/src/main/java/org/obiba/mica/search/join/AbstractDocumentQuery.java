...
public abstract class AbstractDocumentQuery {
  ...
  /**
   * Executes the execute to extracts only study IDs (join field)
   *
   * @return List of study IDs from the aggregation result
   * @throws IOException
   */
  public List<String> queryStudyIds() throws IOException {
    log.info("Query queryStudyIds: {}");
  ...
  private List<String> getResponseStudyIds(List<MicaSearch.AggregationResultDto> aggDtos) {
    ...
    aggDtos.forEach(aggDto -> {
      if(joinFields.getProperty(aggDto.getAggregation().replaceAll("-", ".")) != null) {
