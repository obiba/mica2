<template>
  <q-table
    ...
    :no-data-label="searching ? t('files.no_results') : t('files.empty')"
  ...
  >
    ...
    <template v-slot:body-cell-name="props">
      <q-td :props="props" class="text-no-wrap">
        ...
        <a
          v-if="props.row.permissions?.view"
          href="#"
          class="text-primary"
          @click.prevent="emit('navigate', props.row)"
          >{{ props.value }}</a
        >
    ...
    <template v-slot:body-cell-status="props">
      <q-td :props="props" class="text-no-wrap">
        ...
        <q-icon v-if="isPublished(props.row)" name="star" color="warning" class="q-ml-xs">
          <q-tooltip
            >{{ t('publish.published') }} ({{ getDateLabel(props.row.state?.publicationDate) }} [{{
              props.row.state?.publishedBy
            }}])</q-tooltip
          >
        </q-icon>
...
import { canDelete, fileIcon, isFolder, isPublished, parentPath, sizeLabel } from 'src/utils/files';
...
interface Props {
  ...
  root?: boolean;
  /** the rows are search results: their folder is shown, the parent link is not */
  searching?: boolean;
  loading?: boolean;
}
    ...
    ? [
        {
          ...
          field: (row: FileDto) => parentPath(row.path),
          align: 'left' as const,
          sortable: true,
        },
      ]
